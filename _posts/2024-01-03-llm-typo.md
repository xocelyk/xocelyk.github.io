...
If you use ChatGPT with any regularity, you've probably misspelled words in your prompts and still received fine responses. LLMs are certainly robust to some degree of typos, but not completely robust. [This paper](https://arxiv.org/pdf/2309.11166.pdf) from Wang et al. shows that typos do indeed degrade the helpfulness of responses from foundation models. But as far as I can tell, it remains unclear how much typos hurt LLM performance across tasks, which classes of typos most hurt LLM performance, and how LLMs handle typos in context.

There are two primary reasons why an LLM might still effectively respond to a prompt containing a typo:
...
Try it yourself [here](https://platform.openai.com/tokenizer).
...
We first establish GPT-3.5's baseline accuracy by testing it with typo-free questions. Next, we introduce a typo by altering the source concept within each question and then reassess GPT-3.5's performance on the modified questions. Along with evaluating accuracy, we record two other aspects: the amount of contextual information the sentence offers about the word with the typo, and the complexity level of the introduced typo. To do this, we need a way to measure contextual information and typo complexity.
...
4. Repeated character (10% proability): The current character is typed twice.
